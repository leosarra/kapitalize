...
class EnglishLanguage (private val specialRules: SpecialRules = SpecialRules.NONE) :Language  {
    ...
    private val mcMacExceptions = Regex(".*[acioj]|macevicius|machado|machar|machin|machlin|macias|maciulis|mackie|mackle|macklin|macquarie|macomber|macin|mackintosh|macken|machen|machiel|maciol|mackell|macklem|mackrell|maclin|mackey|mackley|machell|machon")
        ...
        words.forEachIndexed { index, it ->
            run {
                ...
                if (lowercaseWords.matches(word)) {
                    ...
                    if (Regex("van.*").matches(processedInput) && index == 0) builder.append("${word.capitalize()} ") else builder.append("$word ")
                    return@forEachIndexed
                }
